...
BracoServo::BracoServo(int pinoOmbro1, int pinoOmbro2, int pinoCotovelo, int pinoGarra, float L1, float L2) {
    _pinoOmbro1 = pinoOmbro1;
    _pinoOmbro2 = pinoOmbro2; // Salva o pino do segundo servo
...
// Implementação da função iniciar() atualizada
void BracoServo::iniciar() {
    _servoOmbro1.attach(_pinoOmbro1);
    _servoOmbro2.attach(_pinoOmbro2); // Inicia o segundo servo
...
// moverParaPlano não precisa de alterações
...
// Funções de descanso e da garra não precisam de alterações
...
// Função de movimento suave ALTERADA para controlar os dois servos do ombro
void BracoServo::mover_suave(int anguloOmbroFinal, int anguloCotoveloFinal, int anguloGarraFinal) {
    ...
    for (int i = 1; i <= passos; i++) {
        ...
        // Move os dois servos do ombro em direções opostas
        _servoOmbro1.write((int)novoOmbro);
        _servoOmbro2.write(180 - (int)novoOmbro); // Ângulo invertido
    ...
    // Garante que os servos cheguem à posição final exata
    _servoOmbro1.write(_anguloAtualOmbro);
    _servoOmbro2.write(180 - _anguloAtualOmbro); // Ângulo invertido
    _servoCotovelo.write(_anguloAtualCotovelo);
    _servoGarra.write(_anguloAtualGarra);
}

// Função de cinemática inversa não precisa de alterações
